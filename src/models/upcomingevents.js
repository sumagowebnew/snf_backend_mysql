...
function createRecord(recordData, callback) {
    try {
        ...
        db.query('INSERT INTO upcomingevents SET ?', eventData, (err, result) => {
            ...
            db.query('UPDATE upcomingevents SET mainImage = ? WHERE id = ?', [mainImageUrl, eventId], (err, result) => {
                if (err) return callback(err, null);
    ...
    try {
        ...
        db.query('UPDATE upcomingevents SET ? WHERE id = ?', [eventData, id], (err, result) => {
            ...
            if (mainImage) {
                ...
                db.query('UPDATE upcomingevents SET mainImage = ? WHERE id = ?', [mainImageUrl, id], (err, result) => {
                    if (err) return callback(err, null);
                });
            }
